...
@section('content')
                ...
                <tbody>
                    @foreach ($admins as $index => $admin)
                        <tr class="border-b last:border-0 dark:border-gray-700">
                            ...
                            <td class="py-3 px-6" data-fullname-{{ $admin->id }}="{{ $admin->fullname }}">
                                {{ $admin->fullname }}</td>
                            <td class="py-3 px-6"
                                data-phone-number-{{ $admin->id }}="{{ $admin->user->phone_number }}">
                                {{ $admin->user->phone_number }}</td>
                            <td class="py-3 px-6" data-jobtitle-{{ $admin->id }}="{{ $admin->jobtitle }}">
                                {{ $admin->jobtitle }}</td>
                            <td class="py-3 px-6">

                                <button data-modal-target="crud-modal-edit" data-modal-show="crud-modal-edit"
                                    data-id="{{ $admin->id }}"
                                    class="crud-modal-edit-button text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-3 py-1 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 w-full mb-2"
                                    type="button">
                                    Edit
                                </button>
    ...
    {{-- buat tombol edit dan edit gunakan modal --}}
    <div id="crud-modal-edit" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <form class="p-4 md:p-5" action="{{ route('admin.update', $admin->id) }}" method="POST"
                    ...
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2">
                            <label for="name"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama
                                Lengkap</label>
                            <input type="text" name="fullname" id="fullname-edit"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Masukan nama lengkap" required="" value="{{ $admin->fullname }}">
                        </div>
                        <div class="col-span-2 sm:col-span-1">
                            <label for="phone_number"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nomer
                                Telepon</label>
                            <input type="text" name="phone_number" id="phone_number-edit"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                        ...
                        <div class="col-span-2 sm:col-span-1">
                            <label for="jobtitle"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jabatan</label>
                            <input type="text" name="jobtitle" id="jobtitle-edit"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Masukan jabatan" required="" />
                        </div>
                        {{-- add new password dan confirm password --}}
                        <div class="col-span-2 sm:col-span-1">
                            <label for="password"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Password</label>
                            <input type="password" name="password" id="password"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const editModal = document.querySelectorAll('.crud-modal-edit-button');
            editModal.forEach(button => {
                button.addEventListener('click', () => {
                    const id = button.getAttribute('data-id');
                    // set action on form to route('admin.update', id)
                    document.getElementById('crud-modal-edit-form').action =
                        `{{ route('admin.update', ':id') }}`.replace(':id', id);

                    const fullname = document.querySelector(`[data-fullname-${id}]`).getAttribute(
                        `data-fullname-${id}`);
                    const phoneNumber = document.querySelector(`[data-phone-number-${id}]`)
                        .getAttribute(
                            `data-phone-number-${id}`);
                    const jobtitle = document.querySelector(`[data-jobtitle-${id}]`).getAttribute(
                        `data-jobtitle-${id}`);

                    document.getElementById('fullname-edit').value = fullname;
                    document.getElementById('phone_number-edit').value = phoneNumber;
                    document.getElementById('jobtitle-edit').value = jobtitle;
                });
            });
        });
    </script>
