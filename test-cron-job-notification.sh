...
if [ "$response" != "200" ]; then
    print_error "Laravel server is not running. Please start it first:"
...
if echo "$vapid_response" | grep -q '"success":true'; then
    ...
    print_error "VAPID configuration is invalid"
...
stats_response=$(curl -s http://localhost:8000/api/webpush-test/stats)
...
if [ "$total_subscriptions" -gt 0 ]; then
    ...
    echo ""
    echo "To subscribe:"
    echo "1. Open browser: http://localhost:8000/webpush-test-public"
